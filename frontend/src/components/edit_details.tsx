...
function Choose({topic, rq, setTopic, description, setRQ, increment, decrement, helper, scan}: {topic: string, rq: string, setTopic: React.Dispatch<React.SetStateAction<string>>, description: any, setRQ: React.Dispatch<React.SetStateAction<string>>, increment: any, decrement: any, helper: string, scan: boolean}) {
    ...
    return (
        <div className = "flex flex-col justify-self-center">
            <h3 className = "font-bold text-3xl">Set Project Details</h3>
            <p className = "font-light text-sm">{ helper }</p><br/>
            ...
            <div className = "flex gap-1 flex-row">
                ...
                <button disabled = {topic.trim().length <= 0 || (scan === false && rq.trim().length <= 0)} className = "w-fit text-[#f4f4f4] text-lg py-0 px-3" type = "button" onClick = {increment}>→</button>
            </div>
        </div>
    );
}
